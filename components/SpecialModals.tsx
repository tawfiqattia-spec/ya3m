          ...
          <div className="flex-1 overflow-y-auto px-5 md:px-10 py-6 space-y-5 scrollbar-hide">
            <div className="space-y-4">
              {initialItems.map((item, i) => {
                ...
                const showBread = type === 'sandwiches' && !['برجر يا عم', 'حواوشي يا عم', 'طبق فراخ استربس كريسبي', 'صينية سمين مشكل بلدي لفرد واحد', 'صينية شهية لفرد واحد', 'مكرونة بالبشامل لفرد واحد', 'كرات بطاطس بالجبنة لفرد واحد', 'أرز بلبن يا عم'].includes(item.name);
                  ...
                  >
                    <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                      <div className="flex items-center gap-4 w-full sm:w-auto">
                        {item.image && (
                           <motion.div whileHover={{ rotate: 3 }} className="w-16 h-16 shrink-0 rounded-2xl overflow-hidden border border-white/10">
            ...
            <div className="p-6 md:p-8 bg-black/95 backdrop-blur-md border-t border-white/5 shrink-0 pb-10 shadow-[0_-15px_40px_rgba(0,0,0,0.8)]">
              <div className="flex flex-col md:flex-row justify-between items-center gap-5 mb-1">
                ...
                <div className="flex gap-4 w-full md:w-auto">
                    ...
                    <button onClick={() => onFinalSubmit({})} className="flex-1 md:px-8 py-3.5 bg-[#FAB520] text-black rounded-xl font-bold text-xl font-['Lalezar'] shadow-[0_15px_30px_rgba(250,181,32,0.4)] hover:scale-[1.02] transition-all">تثبيت الأكلة</button>
                </div>
              </div>
            </div>
          </div>
